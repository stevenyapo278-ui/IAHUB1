            ...
            <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div
                className={`p-3 rounded-xl border text-[12px] leading-relaxed ${
                  (settings.activeGlpiInstance || 'glpi') === 'glpi'
                    ? 'border-emerald-500/30 bg-emerald-500/5'
                    : 'border-outline-variant/40 bg-surface-container-low/30'
                }`}
              >
                <div className="flex items-center gap-1.5 font-semibold mb-1" style={{ color: '#16a34a' }}>
                  <span className="material-symbols-outlined text-sm">check_circle</span>
                  GLPI Production
                </div>
                <p style={{ color: 'var(--color-on-surface-variant)' }}>
                  Instance réelle utilisée par les techniciens au quotidien.
                  {settings.activeGlpiInstance === 'glpi'
                    ? ' Les tickets créés par la plateforme atterriront directement ici.'
                    : ' Les tickets ne sont PAS envoyés ici actuellement.'}
                </p>
              </div>
              <div
                className={`p-3 rounded-xl border text-[12px] leading-relaxed ${
                  settings.activeGlpiInstance === 'glpi_dev'
              ...
              >
                ...
                <p style={{ color: 'var(--color-on-surface-variant)' }}>
                  Instance de test isolée. Permet de valider le comportement sans impacter la production.
                  {settings.activeGlpiInstance === 'glpi_dev'
                    ? ' Les tickets sont envoyés ici (aucun impact sur la production).'
                    : ' Les tickets ne vont pas ici actuellement.'}
                </p>
              </div>
            </div>

            <div className="mt-3 flex items-start gap-1.5 p-2 rounded-lg" style={{ backgroundColor: 'rgba(59,130,246,0.05)', border: '1px solid rgba(59,130,246,0.12)' }}>
              <span className="material-symbols-outlined text-[14px] shrink-0 mt-px" style={{ color: '#3b82f6' }}>lightbulb</span>
              <p className="text-[11px] leading-relaxed" style={{ color: 'var(--color-on-surface-variant)' }}>
                <strong>Conseil :</strong> En mode <strong>Simulation</strong> ou <strong>Hybride</strong> (depuis le tableau de bord Transition SOS), l'instance DEV est automatiquement sélectionnée pour éviter tout impact sur la production.
              </p>
            </div>
          </motion.div>
        )}
