...
export default function AutomationTab() {
  ...
  return (
    ...
    >
      ...
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {/* SECTION 3 : GESTION DES CONVERSATIONS & MARQUAGE GLPI */}
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      <div className="space-y-md">
        <div className="flex items-center gap-2 border-b border-outline-variant/40 pb-sm">
          <span className="material-symbols-outlined text-primary text-2xl">forum</span>
          <h4 className="font-headline-md text-headline-md text-on-surface font-bold">Gestion des conversations & marquage GLPI</h4>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-lg">
          ...
          >
            <div>
              <div className="font-headline-sm text-headline-sm text-on-surface font-semibold">Date de mise en production</div>
              <p className="font-body-sm text-body-sm text-on-surface-variant mt-1.5">
                Date à partir de laquelle la plateforme a commencé à traiter les emails. Utilisée pour le reporting et les statistiques.
              </p>
            </div>
            <input
              type="date"
              value={settings.goLiveDate ? settings.goLiveDate.slice(0, 10) : ''}
              onChange={(e) => updateSetting('goLiveDate', e.target.value ? `${e.target.value}T00:00:00Z` : null)}
              disabled={saving}
              className={`${inputClass} min-w-[160px] disabled:opacity-50`}
            />
          </motion.div>

          {/* Comportement sur ticket fermé */}
          <motion.div
            variants={itemVariants}
            whileHover={{ y: -1, borderColor: 'var(--color-outline-variant)' }}
            className="bento-card flex items-center justify-between gap-lg p-lg"
          >
            <div>
              <div className="font-headline-sm text-headline-sm text-on-surface font-semibold">Réponse à un ticket fermé</div>
              <p className="font-body-sm text-body-sm text-on-surface-variant mt-1.5">
                Quand un utilisateur répond à un email concernant un ticket déjà fermé.
              </p>
            </div>
            <select
              value={settings.closedTicketBehavior || 'create_new'}
              onChange={(e) => updateSetting('closedTicketBehavior', e.target.value)}
              disabled={saving}
          ...
          >
            <div>
              <div className="font-headline-sm text-headline-sm text-on-surface font-semibold">Seuil de réouverture</div>
              <p className="font-body-sm text-body-sm text-on-surface-variant mt-1.5">
                Au-delà de ce nombre de jours, un ticket fermé ne sera pas rouvert. Un nouveau ticket sera créé à la place.
              </p>
            </div>
            <div className="flex items-center gap-sm shrink-0">
              <input
                ...
                min={1}
                max={730}
                value={settings.reopenThresholdDays ?? 90}
                onChange={(e) => updateSetting('reopenThresholdDays', Math.max(1, Math.min(730, Number(e.target.value) || 90)))}
                disabled={saving}
                className={`${inputClass} w-24 text-center disabled:opacity-50`}
              />
              <span className="font-body-sm text-body-sm text-on-surface-variant font-medium">jours</span>
            </div>
          </motion.div>

          {/* Marqueur de source GLPI */}
          <motion.div
            variants={itemVariants}
            whileHover={{ y: -1, borderColor: 'var(--color-outline-variant)' }}
            className="bento-card flex items-center justify-between gap-lg p-lg"
          >
            <div>
              <div className="font-headline-sm text-headline-sm text-on-surface font-semibold">Marquage des tickets GLPI</div>
              <p className="font-body-sm text-body-sm text-on-surface-variant mt-1.5">
                Distingue les tickets créés automatiquement par la plateforme des tickets créés manuellement dans GLPI.
              </p>
            </div>
            <select
              value={settings.glpiSourceMarker || 'internal_note'}
              onChange={(e) => updateSetting('glpiSourceMarker', e.target.value)}
              disabled={saving}
              className={`${inputClass} min-w-[200px] disabled:opacity-50`}
            >
              <option value="internal_note">Note interne : "Créé automatiquement via SOS Platform"</option>
